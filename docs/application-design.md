...
- 搜索降级条目的人工审核详情必须展示每次搜索查询、搜索服务、状态和返回数量，并按原始
  顺序展示每条候选的标题、可点击 HTTP(S) 链接、已保存摘要和搜索引擎；0 条结果要明确
  呈现，不得与“未执行搜索”混淆；
- 搜索候选只帮助人工调查，不自动成为采用证据或正式 `source_url`。审核员仍须打开链接、
  核对行约束，并通过人工修正或确认未解决完成审计；
